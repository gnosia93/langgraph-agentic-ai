...
* Prompt Guard는 DeBERTa 기반의 300M 규모 경량 분류기로, Prompt Injection과 Jailbreak 탐지에 특화되어 있다. 입력만 검사하며, benign(정상), injection(인젝션), jailbreak(탈옥) 세 가지로 분류한다. 86M으로 가볍기 때문에 CPU만으로도 실행 가능하여, 빠른 1차 필터링 용도로 적합하다.
...
```
prompt-guard 파드와 서비스를 생성하고 파드 배포여부를 확인한다.
```
