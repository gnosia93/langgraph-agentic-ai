...
에이전틱 시스템(Agentic AI)에서 입력값 필터링(Input Filtering)은 시스템의 안전성과 신뢰성을 확보하기 위한 필수적인 안전장치이다. 자율성을 가진 AI 에이전트는 목표 달성을 위해 스스로 데이터를 처리하고 행동하므로, 오염되거나 악의적인 입력값이 들어올 경우 큰 위험이 발생할 수 있다.

* 안전한 행동 및 오작동 방지: 에이전틱 시스템은 행동의 연속성을 가지므로, 잘못된 입력은 추론 및 도구 사용 오류를 일으키고 최종 결과의 정확성을 저하시킨다.
* 프롬프트 인젝션 및 보안 공격 방지: 악의적인 사용자가 AI의 페르소나를 탈취하거나, 시스템 명령어(프롬프트)를 조작하여 보안 취약성을 노리는 인젝션 공격을 시도할 수 있다. 이를 필터링하여 시스템을 보호해야 한다.
* 민감한 정보 및 위험 콘텐츠 제한: 민감한 주제, 개인정보, 혹은 유해한 콘텐츠가 에이전트의 대화나 결과에 포함되지 않도록 시맨틱 필터링이 필요하다.
* 독립적 의사결정의 신뢰성 확보: 에이전트는 다양한 소스에서 방대한 데이터를 수집하여 문제를 분석하고 결정을 내린다. 이 과정에서 유효하지 않거나 잘못된 데이터를 걸러내지 않으면 잘못된 의사결정을 내릴 수 있다.
* 에이전트 간 안전한 상호작용 (A2A): 여러 에이전트가 상호작용(A2A)하는 환경에서는 한 에이전트의 오염된 입력이 다른 에이전트에게 전파될 수 있어 입력 필터링이 필수적이다. 
