        ...
        - name: vllm-worker
          ...
          resources:
            limits:
              nvidia.com/gpu: 8
```
주목할 포인트는 Leader와 Worker의 command가 다르다는 점이다. Leader는 Ray 클러스터를 시작하고 vLLM 서버를 띄우고, Worker는 Leader가 띄운 Ray에 join만 한다. 이 비대칭 구조가 LWS가 필요한 이유 그 자체다.
...
```
이 한 줄로 CRD와 컨트롤러가 설치된다. EKS, GKE, AKS 모두 동일하다.
