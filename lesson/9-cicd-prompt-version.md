...
### Git 브랜치 ###
```
main 브랜치 (프로덕션)         ← 여기에 머지되어야 실제 반영
  │
  └── feature 브랜치         ← 여기에 push한 것 (아직 main에 안 합쳐짐)
        git add
        git commit
        git push           ← Git에는 들어갔지만, feature 브랜치에만 있음
        PR 생성             ← "main에 합쳐주세요" 요청
        자동 평가 실행        ← 여기서 품질 체크
        리뷰 승인
        머지                ← 이때 비로소 main에 반영
```
